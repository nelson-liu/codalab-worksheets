...
1. Auto-generate the migration script. Note that when running `codalab_service.py` with `-d` option (dev mode), this script will be stored at a volume mounted into the Docker container in the `alembic` directory, named with `2019091920_description_of_your_migration_1975ea83b712.py` given the following command:
